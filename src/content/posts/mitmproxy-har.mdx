---
title: mitmproxyでHARファイルに複数のHTTPリクエストを保存する
date: 2025-10-14T12:42:06+09:00
published: true
---

curlコマンドなどで複数リクエストを送信して、それらの複数リクエストのかかった時間などを調査したい場面がありました。
この記事では、mitmproxyを使って立ち上げたproxy server経由で送った複数のリクエストをHARファイルに保存する方法を紹介します。
...
実装上の注意点としては以下の点が挙げられます。

- mitmdumpコマンドを実行してから、proxy serverを起動するまで待つ必要がある
- mitmdumpのプロセスがkillされた時にHARファイルに出力されるので、killコマンドでSIGTERMを送信してから実際にプロセスが終了するまでwaitコマンドで待つ必要がある
